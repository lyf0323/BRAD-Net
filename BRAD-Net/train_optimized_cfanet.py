...
def main():
    ...
    parser.add_argument('--decoder_type', type=str, default='innovative',
                       ...
                       help='decoder type: innovative(推荐,Query+对比学习), ultralight(极小数据集), simplified(无Query), original(基准)')
    ...
    print("=" * 80)
    print("优化版双分支CFANet训练 - Query引导 + 对比学习")
    print("=" * 80)
    print(f"Configuration:")
    print(f"Decoder Type: {opt.decoder_type.upper()}")
    decoder_desc = {
        'innovative': 'Query(100+25) + 对比学习(3层) + 简化MSCA',
        'ultralight': 'Query(50+12) + 对比学习(2层)（资源受限）',
        'simplified': '简化MSCA + 轻量BGRM（无 Query 对照）',
        'original': '原始CFANet（基准）'
    }
    print(f"{decoder_desc.get(opt.decoder_type, '未知')}")
    if opt.decoder_type in ['innovative', 'ultralight']:
        print(f"Query配置: {opt.num_region_queries}个区域 + {opt.num_boundary_queries}个边界")
        print(f"对比学习权重: {opt.weight_contrastive}")
    ...
    print(f"Query自适应聚合 + 对比学习边界-区域解耦 + 简化MSCA")
    print("=" * 80)
    ...
    if opt.decoder_type == 'innovative':
        print(f"创建优化版创新解码器（Query {opt.num_region_queries}+{opt.num_boundary_queries} + 对比学习）")
        ...
        print(f"创建超轻量解码器（Query {opt.num_region_queries}+{opt.num_boundary_queries}）")
        model = create_ultralight_cfanet(
            channel=opt.channel,
            mamba_dim=opt.mamba_dim
        ).cuda()
    else:
        print(f"创建{opt.decoder_type}解码器")
