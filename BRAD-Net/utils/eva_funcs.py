...
def fun_centroid(gt, cuda=True):
    ...
    if gt.sum() == 0:
        X = torch.eye(1, device=device) * round(cols / 2)
        Y = torch.eye(1, device=device) * round(rows / 2)
    
    else:
        total = gt.sum()
        
        # Create tensors on the same device as gt
        i = torch.arange(0, cols, device=device, dtype=torch.float)
        j = torch.arange(0, rows, device=device, dtype=torch.float)
